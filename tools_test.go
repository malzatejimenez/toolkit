...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		if !e.errorExpected {
			path := fmt.Sprintf("./testdata/uploads/%s", uploadedFiles[0].NewFileName)
			if _, err := os.Stat(path); os.IsNotExist(err) {
				t.Errorf("%s: expeted file to exist: %s", e.name, err.Error())
			}
...
func TestTools_UploadOneFile(t *testing.T) {
	...
	path := fmt.Sprintf("./testdata/uploads/%s", uploadedFile.NewFileName)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		t.Errorf("expeted file to exist: %s", err.Error())
	}
...
var jsonTests = []struct {
	...
	{name: "syntax erro in json", json: `{"foo": 1"`, errorExpected: true, maxSize: 1024, allowUnknown: false},
