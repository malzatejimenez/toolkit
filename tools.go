...
const randomStringSource = "abcdefghijklmnopqrstuvwzyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+"
...
// UploadFiles uploads one or more file to a specified directory, and gives files as random name.
// It returns a slice containing the newly named files, the original file names, the size of the files,
// and potentially an error. If the optional last parameter is not set to true, then we will not rename
...
// JSONResponse is the type used for sending a JSON around
	...
	if err != nil {
		...
		case errors.As(err, &unmarshalTypeError):
			if unmarshalTypeError.Field != "" {
				return fmt.Errorf("body contains incorrect JSON typefor field %q", unmarshalTypeError.Field)
			}
